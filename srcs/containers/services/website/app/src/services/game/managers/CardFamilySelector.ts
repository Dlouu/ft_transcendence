...
export class CardFamilySelector extends Container {
	...
	private static readonly BORDER_COLOR: number =
		GAME_CUSTOMIZATION.selector.borderColor;
	private static readonly BORDER_ALPHA: number =
		GAME_CUSTOMIZATION.selector.borderAlpha;
	private static readonly BORDER_WIDTH: number =
		GAME_CUSTOMIZATION.selector.borderWidth;
	private static readonly PIXEL_SIZE: number =
		GAME_CUSTOMIZATION.selector.pixelSize;
	...
	constructor(
		...
		radius: number = GAME_CUSTOMIZATION.selector.defaultRadius,
	...
	private buildPixelArcPoints(
		...
		const steps = Math.max(
			GAME_CUSTOMIZATION.selector.minArcSteps,
			Math.ceil(arcLength / CardFamilySelector.PIXEL_SIZE),
		);
	...
	private snapToPixelGrid(value: number): number {
		return (
			Math.round(value / CardFamilySelector.PIXEL_SIZE) *
			CardFamilySelector.PIXEL_SIZE
		);
	}
