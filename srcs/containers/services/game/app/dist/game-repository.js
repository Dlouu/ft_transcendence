...
const rejoin_game_dto_1 = require("./dto/rejoin-game.dto");
let GameRepositoryService = class GameRepositoryService {
    ...
    create(createGameDto) {
        ...
        const newGame = new UnoGame_1.Game(roomName, players, players.length, botNbr, cardTheme);
        newGame.state = GameEnums_1.GameState.WAITING_FOR_PLAYERS;
    ...
    join(playerId, socket) {
        ...
        if (game.connectedPlayers.has(playerId)) {
            throw new common_1.ConflictException("Player is already connected in this game");
        }
        ...
        if (game.state === GameEnums_1.GameState.PLAYING ||
            game.state === GameEnums_1.GameState.AWAITING_COLOR_CHOICE) {
    ...
    rejoin(player, game) {
        if (!player || !player._socket)
            return;
        game.connectedPlayers.add(player._id);
        const rejoinDto = (0, rejoin_game_dto_1.toRejoinGameDto)(player, game);
        if (!rejoinDto)
            return;
