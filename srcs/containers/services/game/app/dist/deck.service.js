...
let DeckService = class DeckService {
    createDeck() {
        ...
        const numberCards = [
            GameEnums_1.CardCode.One,
            GameEnums_1.CardCode.Two,
            GameEnums_1.CardCode.Three,
            GameEnums_1.CardCode.Four,
            GameEnums_1.CardCode.Five,
            GameEnums_1.CardCode.Six,
            GameEnums_1.CardCode.Seven,
            GameEnums_1.CardCode.Eight,
            GameEnums_1.CardCode.Nine,
        ];
        for (const color of colors) {
            pushCard(GameEnums_1.CardCode.Zero, color);
            for (const numberCard of numberCards) {
                pushCard(numberCard, color);
                pushCard(numberCard, color);
            }
            for (let i = 0; i < 2; i++) {
                pushCard(GameEnums_1.CardCode.Skip, color);
                pushCard(GameEnums_1.CardCode.Reverse, color);
                pushCard(GameEnums_1.CardCode.DrawTwo, color);
            }
        }
        for (let i = 0; i < 4; i++) {
            pushCard(GameEnums_1.CardCode.Wild, GameEnums_1.CardFamily.WILD);
            pushCard(GameEnums_1.CardCode.WildDrawFour, GameEnums_1.CardFamily.WILD);
        }
        return deck;
    }
    discardToDeck(game) {
        ...
        game.deck.setCards(this.shuffleDeck([...game.deck.toArray(), ...game.discard.toArray()]));
        game.discard.clear();
        if (topCard) {
    ...
    startDeal(game) {
        ...
        game.state = GameEnums_1.GameState.DEALING;
